...
#ifndef HASH_REDUCE_H
#define HASH_REDUCE_H
...
template<class Ele, class Keytype> class hash {
 ...
 public:
  ...
  void reset();
  void cleanup();
  void reduce(hash<Ele,Keytype> *other_hash);
};
...
template<class Ele, class Keytype> 
void 
hash<Ele,Keytype>::reduce(hash<Ele,Keytype> *other_hash){
  for (unsigned int i = 0; i < other_hash->my_size; i++) {
    list<Ele,Keytype> *other_list = other_hash->get_list(i);
    
    while (other_list->num_ele() > 0) {
      Ele *other_head = other_list->pop();
      
      Ele *existing_element = lookup(other_head->key());

      if (existing_element)
        existing_element->count += other_head->count;
      else
        insert(other_head);
    }
  }

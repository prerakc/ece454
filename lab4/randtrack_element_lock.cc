...
class sample {
 ...
 public:
  ...
  sample(unsigned the_key){my_key = the_key; count = 0; mutex = PTHREAD_MUTEX_INITIALIZER;};
...
void *process(void *arg) {
  ...
  for (i=args->start; i<args->end; i++){
    ...
    for (j=0; j<SAMPLES_TO_COLLECT; j++){
      ...
      h.unlock(key);
      // LIST CRITICAL SECTION: END

      // ELEMENT CRITICAL SECTION: START
      s->lock();
